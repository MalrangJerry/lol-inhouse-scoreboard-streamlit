...
with st.expander("1) 새 세션 만들기", expanded=True):
    ...
    if st.button("세션 생성", type="primary"):
        ...
        if len(lines) != 10:
            st.error("참가자는 정확히 10명이어야 합니다.")
        else:
            # ✅ 세션 + 참가자 insert 전체를 try/except로 감싸서 원인 출력
            try:
                # 1) 세션 insert
                ...
                # ✅ Supabase(PostgREST)에서 내려준 실제 에러를 화면에 출력
                st.error("Supabase APIError 상세:")
                st.json(e.args[0] if e.args else {"error": "no details"})
                st.stop()
